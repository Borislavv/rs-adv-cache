// Package lfu provides tests for TinyLFU implementation.

#[cfg(test)]
mod tests {
    use crate::config::Admission as AdmissionConfig;
    use crate::storage::lfu::tiny_lfu::ShardedAdmitter;
    use crate::storage::map::NUM_OF_SHARDS;
    ...
    fn test_tiny_lfu_prefers_hot_over_cold() {
        ...
        if hot_rate < 0.85 {
            panic!(
                ...
                100.0 * hot_rate, hot_wins.yes, hot_wins.no
            );
        }
        if cold_rate > 0.15 {
            panic!(
                "cold vs hot admit too high: got={:.2}% want<=15% (yes={} no={})",
                100.0 * cold_rate, cold_wins.yes, cold_wins.no
            );
        }
