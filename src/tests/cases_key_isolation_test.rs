...
use crate::support::{
    assert_equal, assert_ok, cache_addr, do_json, hash, init_test_harness, new_namespace, with_ns,
    H,
};
...
async fn test_key_isolation_path_variants() {
    ...
    // Warm + assert
    let (st1, h1, b1, _) =
        assert_ok(do_json::<RespPayload>("GET", &format!("{}{}", base, p1), &headers).await);
    ...
    let (st2, h2, b2, _) =
        assert_ok(do_json::<RespPayload>("GET", &format!("{}{}", base, p2), &headers).await);
...
async fn test_key_isolation_query_variants() {
    ...
    let (st, _, b, _) =
        assert_ok(do_json::<RespPayload>("GET", &format!("{}{}", base, u1), &headers).await);
    assert_equal(200, st);
    let r1: RespPayload = serde_json::from_slice(&b).unwrap();
    assert_body_matches(&b, &r1, path, &raw_q1, "identity");

    let (st, _, b, _) =
        assert_ok(do_json::<RespPayload>("GET", &format!("{}{}", base, u2), &headers).await);
    assert_equal(200, st);
...
async fn test_key_isolation_header_accept_encoding() {
    ...
    // identity
    let (st, hdr, b, _) =
        assert_ok(do_json::<RespPayload>("GET", &format!("{}{}", base, u), &headers).await);
    assert_equal(200, st);
    ...
    for p in &paths {
        for pr in &projects {
            ...
            for ae in &aes {
                ...
                let (_, _, b, _) = assert_ok(
                    do_json::<RespPayload>("GET", &format!("{}{}", base, u), &headers).await,
                );
        ...
        for j in (0..projects.len()).rev() {
            ...
            for k in (0..aes.len()).rev() {
                ...
                if let Some(want) = seen.get(&key) {
                    if h != want.body_hash {
                        panic!(
                            "leakage or mismatch for {:?}: want body {} got {}",
                            key, want.body_hash, h
                        );
                    }
                }
            }
...
async fn test_header_case_insensitivity() {
    ...
    let (st, _, b1, _) =
        assert_ok(do_json::<serde_json::Value>("GET", &format!("{}{}", base, u), &headers1).await);
    assert_equal(200, st);

    let mut headers2 = H::new();
    headers2.insert("accept-encoding".to_string(), "identity".to_string());
    let (st, _, b2, _) =
        assert_ok(do_json::<serde_json::Value>("GET", &format!("{}{}", base, u), &headers2).await);
    assert_equal(200, st);
...
async fn test_namespace_isolation_changes_body() {
    ...
    let ns_a = format!(
        "{}_A",
        new_namespace("test_namespace_isolation_changes_body")
    );
    let ns_b = format!(
        "{}_B",
        new_namespace("test_namespace_isolation_changes_body")
    ...
    let (st, _, b_a, _) =
        assert_ok(do_json::<serde_json::Value>("GET", &format!("{}{}", base, u_a), &headers).await);
    assert_equal(200, st);
    let (st, _, b_b, _) =
        assert_ok(do_json::<serde_json::Value>("GET", &format!("{}{}", base, u_b), &headers).await);
    assert_equal(200, st);
