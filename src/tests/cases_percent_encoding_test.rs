...
use crate::support::{
    assert_equal, assert_ok, cache_addr, do_json, init_test_harness, new_namespace, phash, H,
};
...
async fn test_query_percent_encoding_space_plus_vs_20_equivalent() {
    ...
    use urlencoding;
    let q_plus = format!(
        "user%5Bid%5D=123&domain=pe.example&language=en&picked=helloworld+foobarbazz&ns={}",
        urlencoding::encode(&ns)
    );
    let q_pct20 = format!(
        "user%5Bid%5D=123&domain=pe.example&language=en&picked=helloworld%20foobarbazz&ns={}",
        urlencoding::encode(&ns)
    );
    ...
    let (st, _, b1, _) = assert_ok(
        do_json::<serde_json::Value>("GET", &format!("{}{}?{}", base, path, q_plus), &headers)
            .await,
    );
    assert_equal(200, st);
    let (st, _, b2, _) = assert_ok(
        do_json::<serde_json::Value>("GET", &format!("{}{}?{}", base, path, q_pct20), &headers)
            .await,
    );
    assert_equal(200, st);
...
async fn test_query_percent_encoding_literal_plus_different_from_space() {
    ...
    let q_plus = format!(
        "user%5Bid%5D=123&domain=pe.example&language=en&picked=helloworld%2Bfoobarbazz&ns={}",
        urlencoding::encode(&ns)
    ); // '+' literal
    ...
    headers.insert("Accept-Encoding".to_string(), "identity".to_string());
    let (st, _, b1, _) = assert_ok(
        do_json::<serde_json::Value>("GET", &format!("{}{}?{}", base, path, q_space), &headers)
            .await,
    );
    assert_equal(200, st);
    let (st, _, b2, _) = assert_ok(
...
async fn test_query_percent_encoding_hex_case_insensitive_equivalent() {
    ...
    use urlencoding;
    let q_lower = format!(
        "user%5Bid%5D=286&domain=pe.example&language=en&picked=a%2fb&ns={}",
        urlencoding::encode(&ns)
    );
    let q_upper = format!(
        "user%5Bid%5D=286&domain=pe.example&language=en&picked=a%2FB&ns={}",
    ...
    headers.insert("Accept-Encoding".to_string(), "identity".to_string());
    let (st, _, b1, _) = assert_ok(
        do_json::<RespPayload>("GET", &format!("{}{}?{}", base, path, q_lower), &headers).await,
    );
    assert_equal(200, st);
    let (st, _, b2, _) = assert_ok(
        do_json::<RespPayload>("GET", &format!("{}{}?{}", base, path, q_upper), &headers).await,
    );
    assert_equal(200, st);
    ...
    if p1.title != p2.title
        ...
        || p1.echo.path != p2.echo.path
        || p1.echo.ae != p2.echo.ae
    {
...
async fn test_query_percent_encoding_utf8_equivalent() {
    ...
    use urlencoding;
    let q_enc = format!(
        "user%5Bid%5D=287&domain=pe.example&language=ru&picked={}&ns={}",
    ...
    headers.insert("Accept-Encoding".to_string(), "identity".to_string());
    let (st, _, b1, _) = assert_ok(
        do_json::<serde_json::Value>("GET", &format!("{}{}?{}", base, path, q_raw), &headers).await,
    );
    assert_equal(200, st);
    let (st, _, b2, _) = assert_ok(
        do_json::<serde_json::Value>("GET", &format!("{}{}?{}", base, path, q_enc), &headers).await,
    );
    assert_equal(200, st);
    if phash(&b1) != phash(&b2) {
...
async fn test_percent_encoding_double_encoding_not_equivalent() {
    ...
    use urlencoding;
    let q_double = format!(
        "user%5Bid%5D=303&domain=pe2.example&language=en&picked=a%252Fb&ns={}",
        urlencoding::encode(&ns)
    ); // -> "a%2Fb"
    let q_single = format!(
        "user%5Bid%5D=303&domain=pe2.example&language=en&picked=a%2Fb&ns={}",
        urlencoding::encode(&ns)
    ); // -> "a/b"
    ...
    headers.insert("Accept-Encoding".to_string(), "identity".to_string());
    let (st, _, b1, _) = assert_ok(
        do_json::<serde_json::Value>("GET", &format!("{}{}?{}", base, path, q_double), &headers)
            .await,
    );
    assert_equal(200, st);
    let (st, _, b2, _) = assert_ok(
        do_json::<serde_json::Value>("GET", &format!("{}{}?{}", base, path, q_single), &headers)
            .await,
    );
    assert_equal(200, st);
    if phash(&b1) == phash(&b2) {
...
async fn test_query_percent_encoding_slash_in_value_equivalent() {
    ...
    use urlencoding;
    let q_enc = format!(
        "user%5Bid%5D=288&domain=pe.example&language=en&picked=a%2Fb&ns={}",
        urlencoding::encode(&ns)
    );
