...
use crate::support::{
    assert_equal, assert_ok, cache_addr, do_json, hash, init_test_harness, new_namespace, H,
};
...
async fn test_query_order_insensitive() {
    ...
    // identity
    let (st, _, b1, _) =
        assert_ok(do_json::<serde_json::Value>("GET", &format!("{}{}", base, u1), &headers).await);
    assert_equal(200, st);
    let (st, _, b2, _) =
        assert_ok(do_json::<serde_json::Value>("GET", &format!("{}{}", base, u2), &headers).await);
    assert_equal(200, st);
    if hash(&b1) != hash(&b2) {
