// Package api provides lifetime manager controller.
...
use crate::storage::SVC_LIFETIME_MANAGER;
...
impl LifetimeManagerController {
    ...
    async fn rate(
        ...
        match controller.orchestrator.cfg(SVC_LIFETIME_MANAGER) {
            Ok(current_cfg) => {
                ...
                match controller.orchestrator.reload(SVC_LIFETIME_MANAGER, new_cfg) {
                    Ok(_) => (
                        StatusCode::OK,
                        [("content-type", "text/plain")],
    ...
    async fn policy(State(controller): State<Arc<Self>>) -> impl IntoResponse {
        ...
        let on_ttl = controller
            ...
            .lifetime()
            .map(|l| {
                if l.is_remove_on_ttl.load(std::sync::atomic::Ordering::Relaxed) {
                    "remove"
                } else {
                    "refresh"
                }
    ...
    async fn to_remove_policy(State(controller): State<Arc<Self>>) -> impl IntoResponse {
        if let Some(lifetime) = controller.cfg.lifetime() {
            lifetime.is_remove_on_ttl.store(true, std::sync::atomic::Ordering::Relaxed);
        }
    ...
    async fn to_refresh_policy(State(controller): State<Arc<Self>>) -> impl IntoResponse {
        if let Some(lifetime) = controller.cfg.lifetime() {
            lifetime.is_remove_on_ttl.store(false, std::sync::atomic::Ordering::Relaxed);
        }
        Self::policy(State(controller)).await
    }
}
